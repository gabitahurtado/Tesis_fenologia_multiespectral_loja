...
# Cargar el archivo modelo_gam6.rds
modelo_gam6 <- readRDS("modelo_gam6.rds")

# Cargar el archivo modelo_gam7.rds
modelo_gam7 <- readRDS("modelo_gam7.rds")

# Cargar el archivo modelo_gam8.rds
modelo_gam8 <- readRDS("modelo_gam8.rds")
...
###### Calcular VIF
library(car)
vif_values <- vif(modelo_gam8)

####### Identificar y eliminar variables/nudos con alto VIF
...
extract_stats <- function(modelo_gam8, data) {
  # Calcular predicciones
  predicted <- predict(modelo_gam8)
...
NDVI_VALIDACION  <- NDVI_CLIMA2 %>%
  anti_join(muestra_para_modelo, by = "id_fila")
...
jpeg("D:/StarMedia/Users/GABY/Desktop/dats_fnls/COD_FN_TSS/anual.jpeg", width = 2500, height = 1600, res=200)
ggplot(muestra_predict, aes(x = year, y = valor_NDVI, color = Epoca)) +
...
temperatura = ggplot(muestra_predict, aes(x = valor_TEMP, y = valor_NDVI) )  +
  ...
  theme(text = element_text(size = 0.8 * 11),  # Ajustar el tamaño del texto (0.8 * tamaño base)
        plot.title = element_text(face = "bold"),
        panel.border = element_rect(colour = "black", fill=NA, linewidth=1),  # Agrega el marco a cada panel
        strip.background = element_rect(fill = "white", colour = "black", size = 1)) + 
